...
<!--
   ...
   KB's  profile for JP2 access copies, created from Geheugen van Nederland migrated JPEGs (A.K.A. KB_ACCESS_LOSSLESS_01/01/2015)
   Johan van der Knijff, KB / National Library of the Netherlands , 24 August 2015.
   
-->
<s:schema xmlns:s="http://purl.oclc.org/dsdl/schematron">
  <s:pattern>
    <s:title>KB access JP2 check Geheugen van Nederland</s:title>
    ...
    <!-- Perhaps leave this out, as this depends on source JPEG -->
    ...
    <!-- Rule looks for one exact match, additional codestream comments are permitted -->
     <s:rule context="/jpylyzer/properties/contiguousCodestreamBox">
        <s:assert test="count(com/comment[text()='KB_ACCESS_LOSSLESS_01/01/2015']) =1">Expected codestream comment string missing</s:assert>
    </s:rule>
     </s:pattern>
</s:schema>

...
<s:pattern>
    ...
    <s:rule context="/j:jpylyzer/j:properties">
        ...
        <!-- check if compression ratio doesn't exceed threshold value (a bit tricky as for images that 
         don't contain much information very high compression ratios may be obtained without losing quality)  
          -->
        <s:assert test="j:compressionRatio &lt; 25">compression ratio too high</s:assert>
        
        <!-- check if compression ratio not smaller than threshold value -->
        <s:assert test="j:compressionRatio &gt; 15">compression ratio too low</s:assert>
    ...
    <s:rule context="/j:jpylyzer/j:properties/j:contiguousCodestreamBox/j:cod">
        ...
        <!-- Layers -->
        <s:assert test="j:layers = '8'">wrong number of layers</s:assert>
        ...
        <s:assert test="j:transformation = '9-7 irreversible'">wrong transformation</s:assert>
